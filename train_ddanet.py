#!/usr/bin/env python
"""
DDANet Training Script for Kvasir-SEG Dataset
Standalone script to train only DDANet model
"""
...
class DDANetTrainer:
    ...
    def val_epoch(self, val_loader, epoch):
        ...
        if avg_dice > self.best_dice:
            ...
            self.patience_counter = 0
            self.save_checkpoint('checkpoints/ddanet_best.pth')
    ...
    def plot_history(self, save_dir='results'):
        ...
        axes[0].set_title('DDANet Training Loss')
        axes[0].legend()
        ...
        axes[1].set_title('DDANet Dice Score')
        axes[1].legend()
        axes[1].grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(f'{save_dir}/ddanet_history.png', dpi=150, bbox_inches='tight')
        print(f'✓ Saved: {save_dir}/ddanet_history.png')
        plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='DDANet Training')
    ...
    print(f"\n{'='*80}")
    print(f"DDANet Training on Kvasir-SEG Dataset (Recommended Model ⭐)")
    print(f"{'='*80}")
    ...
    # Model
    model = DDANet(in_channels=3, out_channels=1).to(DEVICE)
    print(f"[Model] DDANet (Dense Dual Attention Network) initialized")
    ...
    # Trainer
    trainer = DDANetTrainer(model, DEVICE, loss_fn, optimizer, lr_scheduler)
    ...
    with open('results/ddanet_history.json', 'w') as f:
        json.dump(history, f, indent=2)
    print(f"✓ Saved: results/ddanet_history.json")
    ...
    print(f"Best Model: checkpoints/ddanet_best.pth")
    print(f"{'='*80}\n")

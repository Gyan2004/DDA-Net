#!/usr/bin/env python
"""
U-Net Training Script for Kvasir-SEG Dataset
Standalone script to train only U-Net model
"""
...
class UNetTrainer:
    ...
    def val_epoch(self, val_loader, epoch):
        ...
        if avg_dice > self.best_dice:
            ...
            self.patience_counter = 0
            self.save_checkpoint('checkpoints/unet_best.pth')
    ...
    def plot_history(self, save_dir='results'):
        ...
        plt.tight_layout()
        plt.savefig(f'{save_dir}/unet_history.png', dpi=150, bbox_inches='tight')
        print(f'✓ Saved: {save_dir}/unet_history.png')
        plt.close()
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"U-Net Training on Kvasir-SEG Dataset")
    print(f"{'='*80}")
    ...
    print(f"[Model] U-Net initialized")
    ...
    # Trainer
    trainer = UNetTrainer(model, DEVICE, loss_fn, optimizer, lr_scheduler)
    ...
    with open('results/unet_history.json', 'w') as f:
        json.dump(history, f, indent=2)
    print(f"✓ Saved: results/unet_history.json")
    ...
    print(f"Best Model: checkpoints/unet_best.pth")
    print(f"{'='*80}\n")
